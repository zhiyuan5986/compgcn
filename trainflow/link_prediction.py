...
class LinkPrediction(BaseFlow):
    ...
    def __init__(self, args):
        ...
        self.model = CompGCN.build_model_from_args(self.args, self.train_hg).to(self.device)
        # self.model = CompGCN().build_model_from_args(self.args, self.train_hg).to(self.device)
        ...
        if self.args.score_fn == 'distmult':
            ...
            General models do not generate the representations of edge types, so we generate the embeddings of edge types.
            """
            self.r_embedding = nn.ParameterDict({etype[1]: nn.Parameter(torch.Tensor(1, self.args.out_dim))
        ...
        if self.args.test_flag:
            if self.args.dataset in ['HGBl-amazon', 'HGBl-LastFM', 'HGBl-PubMed']:
                ...
                with torch.no_grad():
                    ...
                    h_dict = self.model.input_feature()
                    embedding = self.model(self.train_hg, h_dict)
                    score = torch.sigmoid(self.task.ScorePredictor(self.task.test_hg, embedding, self.r_embedding))
    ...
    def _full_train_setp(self):
        ...
        # NOTE: modify negative_graph
        negative_graph = self.task.construct_negative_graph(self.positive_graph) # initial
        # print(negative_graph.nodes["author"].data['h'])
        # breakpoint()

        #####################################################
        # negative_graph = global_uniform_negative_sampling(self.positive_graph, 60000)
        # negative_graph = dgl.heterograph(
        #     {("author", "ref", "paper"): negative_graph},
        #     num_nodes_dict={"author":self.positive_graph.num_nodes("author"), "paper":self.positive_graph.num_nodes("paper")}
        # )
        #####################################################
    ...
    def _mini_train_step(self, ):
        ...
        for input_nodes, positive_graph, negative_graph, blocks in loader_tqdm:
            ...
            if type(input_nodes) == torch.Tensor:
    ...
    def loss_calculation(self, positive_graph, negative_graph, embedding):
        ...
        p_label = torch.ones(len(p_score), device=self.device)
        n_label = torch.zeros(len(n_score), device=self.device)
        loss = F.binary_cross_entropy_with_logits(torch.cat((p_score, n_score)), torch.cat((p_label, n_label)))
        return loss

    def regularization_loss(self, embedding):
        return torch.mean(embedding.pow(2)) + torch.mean(self.r_embedding.pow(2))
